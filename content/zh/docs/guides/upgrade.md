...
要阻止任何对已经做出的对 OSM 配置的修改，使用 `helm --values` 标志。创建一个[值文件](https://github.com/openservicemesh/osm/blob/{{< param osm_branch >}}/charts/osm/values.yaml)的副本（确认使用了针对升级的图表的版本），改变任何希望定制的值。可以省去所有其他的值。
...
### 其他问题
如果运行中遇到的问题不在上述解决方案之列，请[开一个 GitHub Issue](https://github.com/openservicemesh/osm/issues)。
