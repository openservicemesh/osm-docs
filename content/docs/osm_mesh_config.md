...
```
A list of all the values that can be changed in the MeshConfig can be found in the [`preset-mesh-config` yaml](https://github.com/openservicemesh/osm/blob/main/charts/osm/templates/preset-mesh-config.yaml).
...
| spec.observability.prometheusScraping | bool | `true`| `kubectl patch meshconfig osm-mesh-config -n osm-system -p '{"spec":{"observability":{"prometheusScraping":true}}}'  --type=merge` |
...
| spec.sidecar.initContainerImage | string | `"openservicemesh/init:v0.8.3"` | `kubectl patch meshconfig osm-mesh-config -n osm-system -p '{"spec":{"sidecar":{"initContainerImage":"openservicemesh/envoy-alpine:v0.8.3"}}}'  --type=merge` |
